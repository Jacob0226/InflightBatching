...
class OpenAIServingChat(OpenAIServing):
    ...
    async def chat_completion_stream_generator(
        ...
        # [METRICS] Retrieve the captured server start time; initialize TTFT tracking.
        server_start_time = getattr(request_metadata, "server_start_time", time.time())
                        ...
                        if last_msg_content:
                            for i in range(num_choices):
                                ...
                                data = chunk.model_dump_json(
                                    exclude_unset=True)
                                yield f"data: {data}\n\n"
                    # [METRICS] Capture TTFT when the first tokens are sent.
                    if not first_token_recorded:
                        ttft = time.time() - server_start_time
    ...
    async def chat_completion_full_generator(
        ...
        # [METRICS] Retrieve the server start time for this request.
        server_start_time = getattr(request_metadata, "server_start_time", time.time())
        first_token_recorded = False
        ttft = None  # Time-To-First-Token
        ...
        try:
            async for res in result_generator:
                if not first_token_recorded:
                    # [METRICS] Capture TTFT on the first received response.
                    ttft = time.time() - server_start_time
                    first_token_recorded = True
                final_res = res
        ...
        usage.server_e2e_latency = e2e_latency
